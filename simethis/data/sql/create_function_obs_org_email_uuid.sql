CREATE OR REPLACE FUNCTION flore.obs_org_email_uuid(id_releve integer, sep character varying DEFAULT ' '::character varying)
 RETURNS text
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
DECLARE
obs_org_email_uuid text;
begin
obs_org_email_uuid :=
...
concat_ws($2,
	...
	FROM flore.releve rel
		JOIN referentiels.observateur obs1 on rel.id_obs1 = obs1.id_obs
    	JOIN referentiels.observateur obs2 on rel.id_obs2 = obs2.id_obs
		JOIN referentiels.observateur obs3 on rel.id_obs3 = obs3.id_obs
		JOIN referentiels.observateur obs4 on rel.id_obs4 = obs4.id_obs
		JOIN referentiels.observateur obs5 on rel.id_obs5 = obs5.id_obs
		LEFT JOIN referentiels.organisme orgf on rel.id_org_f = orgf.id_org 
	WHERE rel.id_releve = $1;
	
return obs_org_email_uuid;
